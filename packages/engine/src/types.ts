...
export interface Fire {
  ...
  smold:  number;
  /**
   * อัตราส่วน EF ของ PM2.5 เฟสคุกรุ่นเทียบเฟสเปลวไฟ — ค่าปริยาย 1 (พฤติกรรมเดิม)
   *
   * เดิมโค้ดแยกเฉพาะ**แรงยกตัว** ระหว่างสองเฟส แต่หารมวล PM2.5 ด้วยสัดส่วนมวลเฉยๆ
   * เท่ากับสมมติว่า EF ของสองเฟสเท่ากัน ซึ่งขัดกับที่วัดได้ในสนาม
   *
   * Oanh et al. (2011) Atmos. Environ. 45(2):493-502 · ฟางข้าวในไทย
   *   เผากระจาย (เฟสเปลวไฟเด่น)  4.7 ± 2.2 ก./กก.
   *   เผากอง    (เฟสคุกรุ่นหนัก)  20 ± 8   ก./กก.   → อัตราส่วนราว 4.3 เท่า
   *   ทั้งไฟในแปลงจริง            8.3 ± 2.7 ก./กก.
   *
   * **มวลรวมที่ปล่อยคงเดิมทุกค่า** เพราะ `totalG` ที่ผู้เรียกส่งมาคือค่าที่วัดได้ของทั้งไฟ
   * พารามิเตอร์นี้ย้ายมวลระหว่างสองความสูง (เฟสคุกรุ่นลอยต่ำและกระจายแย่กว่า)
   * ไม่ใช่เพิ่มมวลที่ปล่อย · ค่า <= 0 หรือ NaN ถือเป็น 1
   *
   * ปริยายเป็น 1 เพราะการเปลี่ยนค่านี้ขยับผลลัพธ์ทุกตัว ควรทำหลังมีการตรวจสอบเทียบ
   * ค่าตรวจวัดจริงแล้ว ไม่ใช่ก่อน (ดู docs/model.md หัวข้อความถูกต้อง)
   */
  efRatio?: number;
  rai:    number;
}
