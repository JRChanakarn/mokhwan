  ...
  for(var h=0; h<nH; h++){
    ...
    for(var s2=0; s2<steps; s2++){
      ...
      for(var pi2=puffs.length-1; pi2>=0; pi2--){
        ...
        //
        // ใช้ระยะเทียบเท่าลมแวดล้อมเป็นขั้นต่ำ บนพื้นราบ pf.d ≈ ws·t อยู่แล้ว
        ...
        // เท่าเดิมทุกบิต — golden ของ puffFlat ถูกบันทึกใหม่พร้อมกับ puffTerrain
        //
        // **นี่เป็นตัวแทนเชิงวิศวกรรม ไม่ใช่การคืนนิยามของ Briggs** เส้นโค้ง Briggs ฟิตจาก
        ...
        if(i1 >= i0 && j1 >= j0 && (i1-i0) < 260 && (j1-j0) < 260){
          ...
          if(C.depo){
            // ผลพลอยได้ของการแก้ σ: hef โตตาม sz ด้วย การตกสะสมแห้งของ puff ที่ชะงัก
            // จึงอ่อนลง (ชั้นผสมลึกขึ้น = สัมผัสพื้นน้อยลงต่อหน่วยเวลา) อธิบายได้ทางฟิสิกส์
            // แต่เป็นการเปลี่ยนพฤติกรรมข้อที่สองจากการแก้บรรทัดเดียว จดไว้ให้เห็น
            var tt = pf.t;
