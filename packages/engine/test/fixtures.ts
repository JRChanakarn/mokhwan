 ...
 *     puffTerrain พีค 1069 — เทียบกับ **พื้นราบพารามิเตอร์เดียวกันแต่ไม่มี elev**
 *     (103.8 คำนวณสดใน terrain.test.ts ไม่ใช่ puffFlat ซึ่งมีฝนและพีค 88.26) = ×10.3
 *     · puffFlat พีค/รายชั่วโมง/จุดรับ เท่าเดิมทุกบิต
...
export const PUFF_CASES = {
  ...
  /**
   * ภูมิประเทศ + ลมนิ่ง 0.4 m/s — ฉากเช้ามืดในแอ่งที่เป็นหัวใจของเครื่องมือ
   * ถ้าไม่มี SIGMA_WS_FLOOR ใน puff.ts เคสนี้จะให้ค่าที่พื้นเกือบศูนย์เหมือนบั๊กเดิม
   */
  puffTerrainCalm: (() => {
    const c = buildCase([
      { t: '2026-03-15T05:00', ws: 0.4, wdir: 45, stab: 'F', mix: 150, precip: 0 },
    ], 'puff');
   ...
   * หมายเหตุ: `recMax` ของเคสนี้เป็น [0,0,0] ทั้งก่อนและหลังแก้บั๊ก เพราะจุดรับอยู่ที่
   * 1/3/8 กม. แต่ควันกระจุกในรัศมี ~450 ม. — อย่าอ่านศูนย์สามตัวนี้ว่าเป็นเช็คที่ผ่าน
   */
  puffTerrain: (() => {
    const c = buildCase([
